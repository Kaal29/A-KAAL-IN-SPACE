...
{                                      //de manera directa del juego y no comprometernos con su logica al momento de crear la interfaz
    ...
    public static final Vector2D PLAYER_START_POSITION = new Vector2D(Constant.WIDTH/2 - Assets.ships[ChooseShip.avatar].getWidth()/2,
			Constant.HEIGHT/2 - Assets.ships[ChooseShip.avatar].getHeight()/2);
    ...
    //Para indicar si el jugador a perdido o no
    ...
    public GameState()
    {
        ...
        player = new Player(new Vector2D(Constant.WIDTH/2 - Assets.ships[ChooseShip.avatar].getWidth()/2,
				Constant.HEIGHT/2 - Assets.ships[ChooseShip.avatar].getHeight()/2), new Vector2D(),
				Constant.PLAYER_MAX_VEL, Assets.ships[ChooseShip.avatar], this);
    ...
    public void update()
    {
        ...
        if(gameOver && !gameOverTimer.isRunning()) 
        {
            /**
            //*****************************************
            //Viene de la clase SCoreState
            //Para cuando se cierra la partida anoto el puntaje
            //Pero antes se tiene que leer los puntajes que ya hay en la lista 
            
            try{
                ArrayList<ScoreData> dataList = JSONParser.readFile();

                //Como la partida termino se puede obtener el puntaje
